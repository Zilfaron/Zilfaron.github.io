...
class Game {
	...
	start() {
		...
		localStorage.setItem("last-game", (new Date()).toLocaleString());
	...
	nextWave() {
		...
		if (this.current_wave >= this.waves) {
			this.win();
			return false;
		}
	...
	win() {
		alert("You win :)");
		localStorage.removeItem("last-scores");
		localStorage.setItem("last-scores", `${this.current_wave}/${this.waves}`);		
		location.reload();
	}
